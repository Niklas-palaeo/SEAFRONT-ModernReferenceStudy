...
for (shell_name in shells) {
  ...
  d18O_plot <- 
    ...
    labs(x = "Distance to edge by sample number", y = bquote(delta^18*O~ (VPDB)),title = paste(shell_name)) +
    theme_cowplot()
  ...
  MgCa_plot <- 
    ...
    labs(x = "Distance to edge [mm]", title = paste(shell_name),y="Mg/Ca intensity ratio (a.u.)") +
    theme_cowplot()
...
final_plot_A <- plot_grid(plotlist = plot_list[1:6], ncol = 1)
final_plot_B <- plot_grid(plotlist = plot_list[7:10], ncol = 1)
final_plot_C <- plot_grid(plotlist = plot_list[11:14], ncol = 1)
...
ModSeq %>% ungroup() %>% select(shell) %>% unique()



Plot_Corr <- Correlation %>% 
  filter(!(shell=="FR-P_A"|shell=="FR-P_B"|shell=="FR-P_C")) %>% 
